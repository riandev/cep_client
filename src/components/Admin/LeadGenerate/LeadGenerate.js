...
const LeadGenerate = () => {
    ...
    fetch("http://192.168.10.11:5003/initialLead?initDate=" + initialDate)
      ...
      .then((data) => {
        setInitialLeads(
          data
            .map((x) => {
              return [...x.freeSampling, ...x.packetSales];
            })
            .flat()
            .map((a) => {
              return { ...a, for_d: "d" };
  ...
  const updateInitialLeads = () => {
    fetch("http://192.168.10.11:5003/updateInitialLead", {
      method: "PATCH",
  ...
  const regenerateUpdate = () => {
    fetch("http://192.168.10.11:5003/regenerateUpdate", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
              ...
              <div className="mt-3 d-flex justify-content-between">
                <button className="btn btn-outline-primary">
                  <CSVLink
                    ...
                    filename="AKTCL_CEP_InitialLead.csv"
                    data={initialLeads}
                  >
                    Download
                  </CSVLink>
